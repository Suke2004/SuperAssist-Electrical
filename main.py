...
# Windows consoles default to cp1252; every emoji-bearing print would raise
# UnicodeEncodeError and kill the caller (the monitoring pump included).
# Force UTF-8 with replacement so logging can never crash the app.
...
class GlobalCommandMonitor:
    ...
    def _process_command(self, command_data):
        ...
        # Check for duplicate commands within cooldown period (exempt scroll for continuous smoothness)
        if (command != 'scroll' and
            self.last_processed_command and 
    ...
    def dispatch_command(self, command_data):
        """Execute global command immediately in-memory without disk polling latency"""
        try:
...
def setup_webview_window():
    ...
    def on_window_shown():
        ...
        # Find window and configure always-on-top and hide from taskbar
        if window_manager.find_aura_window():
            print("🔍 Window found - setting up always-on-top and taskbar stealth")
            
            # Hide from taskbar by default for stealth
            window_manager.window_manager.hide_from_taskbar()
            ...
            print("⚠️ Could not find window for window management")
        
        # Wire in-memory callback for microsecond hotkey dispatch
        window_manager.window_manager.command_callback = command_monitor.dispatch_command
    ...
    def on_window_closing():
        ...
        window_manager.window_manager.stop_hotkey_listener()
...
def main():
    """Main application entry point"""
    print("🚀 Starting SuperAssist — Electrical Engineering Interview Copilot...")
