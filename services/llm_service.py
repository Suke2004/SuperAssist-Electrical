    ...
    async def get_ai_answer(self, question: str, stream_callback=None, generate_full_answers: Optional[bool] = None) -> Tuple[str, Dict[str, Any]]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Default reasoning_effort to 'low' for Groq/OpenAI to eliminate multi-second thinking stalls
                if self.provider_name in ("Groq", "OpenAI") and "reasoning_effort" not in api_params:
                ...
                if use_streaming:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if "reasoning_effort" in str(req_err).lower() and "reasoning_effort" in api_params:
