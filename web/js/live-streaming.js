...
export class LiveStreaming {
    constructor(config = {}) {
        this.config = {
            ...
            streamingSpeed: 6, // ultra-responsive word streaming
            aiStreamingSpeed: 2, // near-instant AI rendering
            enableMarkdown: true, // Enable markdown processing
            markdownSpeed: 6, // fast markdown elements
            ...config
        };
    ...
    addCodeBlock(container, code, language) {
        ...
        if (lang === 'svg' || (code && code.trim().startsWith('<svg'))) {
            const svgHtml = this.markdownProcessor.generateSvgDiagramHTML({ language: 'svg', content: code });
    ...
    displayInstantMarkdown(container, content) {
        try {
            ...
            blocks.forEach(block => {
                if (block.type === 'svg' || (block.type === 'code' && ((block.language || '').toLowerCase() === 'svg' || (block.content && block.content.trim().startsWith('<svg'))))) {
                    this.addCodeBlock(container, block.content, 'svg');
