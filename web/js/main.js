...
async function startInterview() {
    ...
    if (!processingStarted) {
        console.error("❌ Could not start audio streams. Please check permissions and try again.");
        switchView('preflight');
        return;
    }
...
// Delegated copy and view handlers for all code blocks and SVG diagrams
function setupGlobalCopyHandler() {
    document.addEventListener('click', async (e) => {
        // 0. Toggle Textbook vs AI Schematic View Tabs
        const viewTabBtn = e.target.closest('.svg-view-tab-btn');
        if (viewTabBtn) {
            const container = viewTabBtn.closest('.ee-svg-diagram-container');
            if (container) {
                const view = viewTabBtn.getAttribute('data-view');
                const tbView = container.querySelector('.ee-view-textbook');
                const aiView = container.querySelector('.ee-view-ai');
                const codeView = container.querySelector('.ee-svg-code-view');
                const toggleBtn = container.querySelector('.svg-toggle-btn');
                
                // Hide code view if it was open
                if (codeView) codeView.style.display = 'none';
                if (toggleBtn) toggleBtn.textContent = '🔍 Code';

                container.querySelectorAll('.svg-view-tab-btn').forEach(b => b.classList.remove('active'));
        ...
        // 1. Toggle SVG code / diagram view
        const toggleBtn = e.target.closest('.svg-toggle-btn');
        if (toggleBtn) {
            const container = toggleBtn.closest('.ee-svg-diagram-container');
            if (container) {
                const codeView = container.querySelector('.ee-svg-code-view');
                const tbView = container.querySelector('.ee-view-textbook');
                const aiView = container.querySelector('.ee-view-ai');
                const defaultViewport = container.querySelector('.ee-svg-viewport:not(.ee-view-textbook):not(.ee-view-ai)');
                
                if (codeView) {
                    const isCodeHidden = codeView.style.display === 'none';
                    codeView.style.display = isCodeHidden ? 'block' : 'none';
                    toggleBtn.textContent = isCodeHidden ? '🖼️ Diagram' : '🔍 Code';
                    
                    if (isCodeHidden) {
                        // Showing code: hide viewports
                        if (tbView) tbView.style.display = 'none';
                        if (aiView) aiView.style.display = 'none';
                        if (defaultViewport) defaultViewport.style.display = 'none';
                    } else {
                        // Restoring diagram: show active tab
                        const activeTab = container.querySelector('.svg-view-tab-btn.active');
                        const activeView = activeTab ? activeTab.getAttribute('data-view') : 'textbook';
                        if (activeView === 'ai' && aiView) {
                            aiView.style.display = 'flex';
                        } else if (tbView) {
                            tbView.style.display = 'flex';
                        } else if (defaultViewport) {
                            defaultViewport.style.display = 'flex';
                        }
                    }
                }
            }
        ...
        // 2. Copy SVG diagram markup
        const svgCopyBtn = e.target.closest('.svg-copy-btn');
        if (svgCopyBtn) {
            const container = svgCopyBtn.closest('.ee-svg-diagram-container');
            if (container) {
                const svgElem = container.querySelector('.ee-svg-viewport svg');
                const codeElem = container.querySelector('.ee-svg-code-view code');
                const textToCopy = svgElem ? svgElem.outerHTML : (codeElem ? codeElem.textContent : '');
                if (textToCopy) {
                    try {
                        ...
                        console.warn('SVG copy failed:', err);
                    }
                }
            }
            return;
