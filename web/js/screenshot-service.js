...
class ScreenshotService {
    ...
    showVisionModeIndicator() {
        ...
        // Log to console for stealth
        console.log('👁️ Vision Mode Active (Global)');
        
        // Suppress on-screen banner during live interview for stealth
        const liveViewActive = document.getElementById('live-view')?.classList.contains('active');
        if (liveViewActive) {
            return;
        }
    ...
    showNotification(title, message, type = 'info') {
        ...
        console.log(`[Capture - ${type.toUpperCase()}] ${title}: ${message}`);
        
        // Suppress on-screen popup during live interview for stealth
        const liveViewActive = document.getElementById('live-view')?.classList.contains('active');
        if (liveViewActive) {
            return;
        }
