    ...
    handleOnboarding() {
        ...
        for (const [id, name] of Object.entries(requiredFields)) {
            ...
            if (!element || !element.value) {
                console.warn(`[Validation] ${name} is a required field.`);
                if (element) {
                    element.focus();
        ...
        if (focusCheckboxes.length === 0) {
            console.warn('[Validation] Please select at least one Interview Focus.');
            allValid = false;
        }
        ...
        if ((secondaryProvider && !secondaryModel) || (!secondaryProvider && secondaryModel)) {
            console.warn('[Validation] If you select a secondary provider, you must also select a secondary model (or leave both empty).');
            return false;
        }
        ...
        if (secondaryProvider && secondaryModel) {
            ...
            if (primaryProvider === secondaryProvider && primaryModel === secondaryModel) {
                console.warn('[Validation] Primary and secondary AI models cannot be the same. Please select different models.');
                return false;
            }
        }
        ...
        if ((visionProvider && !visionModel) || (!visionProvider && visionModel)) {
            console.warn('[Validation] If you select a vision provider, you must also select a vision model (or leave both empty).');
            return false;
        }
        ...
        if ((secondaryVisionProvider && !secondaryVisionModel) || (!secondaryVisionProvider && secondaryVisionModel)) {
            console.warn('[Validation] If you select a secondary vision provider, you must also select a secondary vision model (or leave both empty).');
            return false;
        }
