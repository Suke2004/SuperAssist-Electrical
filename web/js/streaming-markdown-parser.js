...
export class StreamingMarkdownParser {
    ...
    shouldReprocess() {
        ...
        if (
            ...
            newContent.includes('\\[') ||                           // Display math bracket
            newContent.includes('\\(') ||                           // Inline math paren
            newContent.includes('<svg') ||                          // SVG diagram start
            newContent.includes('</svg>') ||                        // SVG diagram end
    ...
    generateCodeBlockHTML(block) {
        const language = (block.language || 'javascript').trim().toLowerCase();
        if (block.type === 'svg' || language === 'svg' || (block.content && block.content.trim().startsWith('<svg'))) {
            return this.markdownProcessor.generateSvgDiagramHTML(block);
        }
